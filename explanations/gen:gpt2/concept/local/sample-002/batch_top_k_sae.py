...
from interpreto.concepts import BatchTopKSAEConcepts, TopKInputs
from interpreto.concepts.methods.overcomplete import DeadNeuronsReanimationLoss

device = "cuda" if torch.cuda.is_available() else "cpu"

splitter = SplitterForGeneration('gpt2', split_point=8, batch_size=8, device_map=device)
raw_inputs = load_dataset('wikimedia/wikipedia', '20231101.en')['train']["text"][:10000]
# Truncate to the model's context window so `TopKInputs.interpret` and
# `get_activations` agree on the same tokenization.
inputs = [splitter.tokenizer.decode(splitter.tokenizer(t, truncation=True, max_length=1024, add_special_tokens=False)["input_ids"], skip_special_tokens=True) for t in raw_inputs]
